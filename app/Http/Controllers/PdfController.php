...
class PdfController extends Controller
{
    ...
    {
        $isCustomerReceipt = $payment->transaction_type === 'receipt' && $payment->party_type === 'customer';

        abort_unless(
            $isCustomerReceipt
                ? ($request->user()?->can('manage_receipts') || $request->user()?->can('manage_payments'))
                : $request->user()?->can('manage_payments'),
            403
        );
