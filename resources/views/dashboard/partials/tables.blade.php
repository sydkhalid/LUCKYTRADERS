...
<div class="row g-4 lt-dashboard-table-grid">
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                <span class="avatar rounded bg-label-primary lt-table-icon">
                ...
                <div>
                    ...
                    <small class="text-muted">Latest customer invoices</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    <thead><tr><th>Invoice</th><th>Customer</th><th>Type</th><th class="text-end">Total</th></tr></thead>
                    <tbody>
                        @forelse ($tables['recent_sales'] as $sale)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $sale->sale_no }}</td>
                                <td>{{ $sale->customer?->name ?: '-' }}</td>
                                <td><span class="badge bg-label-primary">{{ strtoupper(str_replace('_', '-', $sale->bill_type)) }}</span></td>
                                <td class="text-end fw-semibold text-heading">{{ $money($sale->total_amount) }}</td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                <span class="avatar rounded bg-label-warning lt-table-icon">
                ...
                <div>
                    ...
                    <small class="text-muted">Latest supplier bills</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['recent_purchases'] as $purchase)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $purchase->purchase_no }}</td>
                                <td>{{ $purchase->supplier?->name ?: '-' }}</td>
                                <td><span class="badge bg-label-warning">{{ strtoupper(str_replace('_', '-', $purchase->bill_type)) }}</span></td>
                                <td class="text-end fw-semibold text-heading">{{ $money($purchase->total_amount) }}</td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                <span class="avatar rounded bg-label-danger lt-table-icon">
                ...
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['low_stock_products'] as $product)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $product->name }}</td>
                                <td>{{ $product->category?->name ?: '-' }}</td>
                                <td class="text-end"><span class="badge bg-label-danger">{{ $number($product->current_stock) }} {{ $product->unit }}</span></td>
                                <td class="text-end fw-semibold text-heading">{{ $money((float) $product->current_stock * (float) $product->purchase_price) }}</td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                <span class="avatar rounded bg-label-info lt-table-icon">
                ...
                <div>
                    ...
                    <small class="text-muted">Highest sales contribution</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['top_customers'] as $customer)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $customer->name }}</td>
                                <td class="text-end">{{ $customer->invoices_count }}</td>
                                <td class="text-end fw-semibold text-heading">{{ $money($customer->total_amount) }}</td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                <span class="avatar rounded bg-label-success lt-table-icon">
                ...
                <div>
                    ...
                    <small class="text-muted">Latest cash and bank receipts</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['recent_payments'] as $payment)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $payment->payment_no }}</td>
                                <td>{{ $payment->party_name }}</td>
                                <td><span class="badge bg-label-success">{{ strtoupper($payment->payment_mode) }}</span></td>
                                <td class="text-end fw-semibold text-heading">{{ $money($payment->amount) }}</td>
                            </tr>
                        @empty
        ...
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                ...
                <div>
                    ...
                    <small class="text-muted">Open finance balances</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['active_loans'] as $loan)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $loan->loan_no }}</td>
                                <td>{{ $loan->party_name }}</td>
                                <td><span class="badge bg-label-primary">{{ $loan->typeLabel() }}</span></td>
                                <td class="text-end fw-semibold text-danger">{{ $money($loan->balance_amount) }}</td>
                            </tr>
                        @empty
        ...
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                ...
                <div>
                    ...
                    <small class="text-muted">Receivables due</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['pending_customer_payments'] as $sale)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $sale->sale_no }}</td>
                                <td>{{ $sale->customer?->name ?: '-' }}</td>
                                <td class="text-end fw-semibold text-danger">{{ $money($sale->balance_amount) }}</td>
                            </tr>
                        @empty
    ...
    <div class="col-xl-6">
        <section class="card h-100 lt-table-card sneat-table-card">
            <div class="card-header lt-table-card-header">
                ...
                <div>
                    <h5 class="card-title mb-0">Pending Supplier Payments</h5>
                    <small class="text-muted">Payables due</small>
                </div>
            </div>
            <div class="table-responsive text-nowrap">
                <table class="table table-hover lt-dashboard-table mb-0">
                    ...
                    <tbody>
                        @forelse ($tables['pending_supplier_payments'] as $purchase)
                            <tr>
                                <td class="fw-semibold text-heading">{{ $purchase->purchase_no }}</td>
                                <td>{{ $purchase->supplier?->name ?: '-' }}</td>
                                <td class="text-end fw-semibold text-danger">{{ $money($purchase->balance_amount) }}</td>
                            </tr>
                        @empty
