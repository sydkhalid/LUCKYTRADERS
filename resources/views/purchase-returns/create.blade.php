...
@section('content')
    <div class="max-w-6xl">
        <div class="mb-5 flex items-center justify-between">
            <div>
                <h2 class="text-lg font-semibold text-gray-900">Create Purchase Return</h2>
                <p class="text-sm text-gray-500">Returned purchase goods decrease stock and reduce payable or record refund received.</p>
            </div>
        ...
        <form method="POST" action="{{ route('purchase-returns.store') }}" class="rounded bg-white p-6 shadow" data-ajax-form>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-gray-700">Purchase</label>
                    <select name="purchase_id" id="sourceId" class="w-full rounded border-gray-300 shadow-sm focus:border-slate-500 focus:ring-slate-500" required>
                        <option value="">Select purchase</option>
                        @foreach ($purchases as $purchase)
                            <option value="{{ $purchase->id }}" @selected((int) old('purchase_id', request('purchase_id')) === $purchase->id)>
                                {{ $purchase->purchase_no }} - {{ $purchase->supplier?->name }} - Balance Rs. {{ number_format((float) $purchase->balance_amount, 2) }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mt-5 grid grid-cols-1 gap-4 rounded bg-slate-50 p-4 text-sm md:grid-cols-4">
                <div>
                    ...
                    <p class="mt-1 font-semibold text-gray-900" id="selectedCustomerText">Select purchase</p>
                </div>
                <div>
                ...
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Purchase Balance</p>
            ...
            <div class="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
                ...
                <div class="rounded bg-slate-50 p-5">
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="text-xs text-gray-500">Allocate full return total between adjustment and refund received.</div>
                        <div><label class="mb-1 block text-sm font-medium text-gray-700">Adjustment Amount</label><input type="number" name="adjustment_amount" value="{{ old('adjustment_amount', '0') }}" min="0" step="0.01" class="w-full rounded border-gray-300 text-right shadow-sm focus:border-slate-500 focus:ring-slate-500" required></div>
                        <div><label class="mb-1 block text-sm font-medium text-gray-700">Refund Amount</label><input type="number" name="refund_amount" value="{{ old('refund_amount', '0') }}" min="0" step="0.01" class="w-full rounded border-gray-300 text-right shadow-sm focus:border-slate-500 focus:ring-slate-500" required></div>
                        <div>
            ...
            <div class="mt-6 flex justify-end gap-3">
                <a href="{{ route('purchase-returns.index') }}" class="rounded border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</a>
                <button class="rounded bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">Save Purchase Return</button>
            </div>
        </form>
    </div>
