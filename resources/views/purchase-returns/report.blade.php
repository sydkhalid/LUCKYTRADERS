...
@section('content')
    <div class="mb-5 flex items-center justify-between">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">Purchase Return Report</h2>
            <p class="text-sm text-gray-500">Date filtered supplier return report.</p>
        </div>
        <a href="{{ route('purchase-returns.index') }}" class="rounded border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Purchase Returns</a>
    </div>

    <form method="GET" action="{{ route('purchase-returns.report') }}" class="mb-5 rounded bg-white p-5 shadow">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            ...
            <div class="flex items-end"><a href="{{ route('purchase-returns.report') }}" class="w-full rounded border border-gray-300 px-4 py-2 text-center text-sm font-semibold text-gray-700 hover:bg-gray-50">Reset</a></div>
        </div>
    </form>
    ...
    <div class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Refund</p><h3 class="mt-1 text-xl font-bold text-emerald-700">₹ {{ number_format((float) $totals['refund'], 2) }}</h3></div>
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Adjustment</p><h3 class="mt-1 text-xl font-bold text-red-700">₹ {{ number_format((float) $totals['adjustment'], 2) }}</h3></div>
    </div>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($returns as $return)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right text-emerald-700">₹ {{ number_format((float) $return->refund_amount, 2) }}</td>
                        <td class="px-4 py-3 text-right text-red-700">₹ {{ number_format((float) $return->adjustment_amount, 2) }}</td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-500">No purchase returns found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
