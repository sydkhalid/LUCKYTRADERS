...
@section('content')
    ...
    <div class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div class="rounded bg-white p-5 shadow"><p class="text-sm text-gray-500">Refund</p><h3 class="mt-1 text-xl font-bold text-emerald-700">₹ {{ number_format((float) $return->refund_amount, 2) }}</h3></div>
        <div class="rounded bg-white p-5 shadow"><p class="text-sm text-gray-500">Adjustment</p><h3 class="mt-1 text-xl font-bold text-red-700">₹ {{ number_format((float) $return->adjustment_amount, 2) }}</h3></div>
    </div>
