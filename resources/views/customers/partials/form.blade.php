...
<form method="POST" action="{{ $action }}">
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div>
            ...
            <input type="text" name="phone" value="{{ old('phone', $customer->phone ?? '') }}" class="w-full rounded border-gray-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
        </div>

        <div>
            <label class="mb-1 block text-sm font-medium text-gray-700">Email</label>
            <input type="email" name="email" value="{{ old('email', $customer->email ?? '') }}" class="w-full rounded border-gray-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
        </div>

        <div>
            <label class="mb-1 block text-sm font-medium text-gray-700">GST Number</label>
            <input type="text" name="gst_number" value="{{ old('gst_number', $customer->gst_number ?? '') }}" class="w-full rounded border-gray-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">
        </div>
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="address" rows="3" class="w-full rounded border-gray-300 shadow-sm focus:border-slate-500 focus:ring-slate-500">{{ old('address', $customer->address ?? '') }}</textarea>
        </div>
    </div>

    <div class="mt-6 flex justify-end gap-3">
        <a href="{{ route('customers.index') }}" class="rounded border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancel</a>
        <button class="rounded bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">Save Customer</button>
    </div>
</form>
