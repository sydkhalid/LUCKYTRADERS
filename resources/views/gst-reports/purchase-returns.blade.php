...
@section('content')
    ...
    <div class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div class="erp-summary-card"><p class="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Input GST Adjustment</p><h3 class="mt-2 text-xl font-black text-emerald-700">₹ {{ number_format((float) $totals['gst'], 2) }}</h3></div>
        <div class="erp-summary-card"><p class="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Total Debit Notes</p><h3 class="mt-2 text-xl font-black text-slate-950">₹ {{ number_format((float) $totals['total'], 2) }}</h3></div>
        <div class="erp-summary-card"><p class="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Refund Received</p><h3 class="mt-2 text-xl font-black text-emerald-700">₹ {{ number_format((float) $totals['refund'], 2) }}</h3></div>
        <div class="erp-summary-card"><p class="text-xs font-black uppercase tracking-[0.16em] text-slate-500">Payable Adjusted</p><h3 class="mt-2 text-xl font-black text-red-700">₹ {{ number_format((float) $totals['adjustment'], 2) }}</h3></div>
    </div>
