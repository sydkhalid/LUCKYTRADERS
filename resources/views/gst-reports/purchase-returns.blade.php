...
@section('content')
    <div class="mb-5 flex items-center justify-between">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">GST Purchase Return / Debit Note Report</h2>
            <p class="text-sm text-gray-500">Only debit notes linked to GST purchases are included.</p>
        </div>
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('gst-reports.export', array_merge($filters, ['type' => 'purchase_returns'])) }}" class="rounded bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-700">Export CSV</a>
        </div>
    </div>
    ...
    <div class="mb-5 grid grid-cols-1 gap-4 md:grid-cols-5">
        ...
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Input GST Adjustment</p><h3 class="mt-1 text-xl font-bold text-emerald-700">Rs. {{ number_format((float) $totals['gst'], 2) }}</h3></div>
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Total Debit Notes</p><h3 class="mt-1 text-xl font-bold text-gray-900">Rs. {{ number_format((float) $totals['total'], 2) }}</h3></div>
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Refund Received</p><h3 class="mt-1 text-xl font-bold text-emerald-700">Rs. {{ number_format((float) $totals['refund'], 2) }}</h3></div>
        <div class="rounded bg-white p-4 shadow"><p class="text-sm text-gray-500">Payable Adjusted</p><h3 class="mt-1 text-xl font-bold text-red-700">Rs. {{ number_format((float) $totals['adjustment'], 2) }}</h3></div>
    </div>
    ...
    <div class="overflow-hidden rounded bg-white shadow">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-slate-100 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">
                <tr>
                    <th class="px-4 py-3">Debit Note No</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($returns as $return)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-700">{{ $return->purchase?->purchase_no ?: '-' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $return->purchase?->supplier_invoice_no ?: '-' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $return->supplier?->name ?: '-' }}</td>
                        <td class="px-4 py-3 text-gray-700">{{ $return->supplier?->gst_number ?: '-' }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="11" class="px-4 py-8 text-center text-gray-500">No GST purchase returns found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
