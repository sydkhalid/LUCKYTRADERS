...
<head>
    <meta charset="UTF-8">
    <title>{{ $return->return_no }} - Credit Note</title>
...
<body class="bg-gray-100 text-gray-900">
    @php($isGst = $return->sale?->bill_type === 'gst')
    ...
    <div class="no-print mx-auto my-6 flex max-w-5xl justify-end gap-2">
        ...
        <a href="{{ route('sales-returns.show', $return) }}" class="rounded border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700">Back</a>
    </div>
    ...
    <main class="mx-auto max-w-5xl bg-white p-8 shadow print:shadow-none">
        <header class="border-b-2 border-slate-900 pb-5">
            <div class="flex items-start justify-between gap-6">
                ...
                <div class="text-right">
                    <h2 class="text-xl font-bold text-slate-900">CREDIT NOTE</h2>
                    <p class="mt-2 text-sm text-gray-700">Credit Note No: <span class="font-semibold">{{ $return->return_no }}</span></p>
                    <p class="text-sm text-gray-700">Date: <span class="font-semibold">{{ $return->return_date?->format('d M Y') }}</span></p>
                    <p class="text-sm text-gray-700">Original Invoice: <span class="font-semibold">{{ $return->sale?->sale_no }}</span></p>
                </div>
            </div>
        </header>
        ...
        <section class="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2">
            <div class="rounded border border-gray-200 p-4">
                <p class="text-xs font-semibold uppercase text-gray-500">Customer</p>
                <h3 class="mt-2 text-lg font-semibold text-gray-900">{{ $return->customer?->name }}</h3>
                <p class="mt-1 text-sm text-gray-700">{{ $return->customer?->address ?: '-' }}</p>
                <p class="text-sm text-gray-700">Phone: {{ $return->customer?->phone ?: '-' }}</p>
                @if ($isGst)
                    <p class="text-sm text-gray-700">GST: {{ $return->customer?->gst_number ?: '-' }}</p>
                @endif
            </div>
            <div class="rounded border border-gray-200 p-4">
