...
@section('content')
    <x-erp.page-header
        title="Sales Returns"
        description="Customer returned goods with stock, ledger, and refund posting."
        kicker="Returns"
    >
        <x-slot:actions>
            <a href="{{ route('sales-returns.report') }}" class="erp-secondary-button">Report</a>
            <a href="{{ route('sales-returns.create') }}" class="erp-primary-button">Create Return</a>
        </x-slot:actions>
    </x-erp.page-header>
    ...
    <form id="salesReturnFilters" class="mb-5 grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-4">
        <div>
    ...
    <x-erp.datatable
        id="salesReturnsTable"
        :ajax-url="route('erp.datatables', 'sales-returns')"
        filter-form="#salesReturnFilters"
        search-placeholder="Search return, sale, customer..."
    ...
    >
        <thead>
            <tr>
                ...
                <th class="px-4 py-3" data-column="sale" data-orderable="false" data-searchable="false">Sale</th>
